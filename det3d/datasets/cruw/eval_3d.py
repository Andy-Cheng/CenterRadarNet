...
from det3d.datasets.cruw.rotate_iou import rotate_iou_gpu_eval, d3_box_overlap_kernel
import numba
import math
...
class CRUW3DEvaluator():
    def __init__(
        ...
        dataset_dicts,
        iou_thresholds,
        class_names = ['Car', 'BusorTruck'],
    ):

        self._dataset_dicts = {}
        for dikt in dataset_dicts:
            seq, frame, rdr_frame = dikt.pop('seq'), dikt.pop('frame'), dikt.pop('rdr_frame')
    ...
    def process(self, preds):
        ...
        for key, val in preds.items():
            ...
            for anno in gt_dataset_dict:
                ...
                L, H, W = anno['scale']
                x, y, z = anno['position']
                # rot_y = box_np_ops.limit_period(anno['euler'][2], offset=0.5, period=np.pi * 2)
                rot_y = box_np_ops.limit_period(anno['euler'][1], offset=0.5, period=np.pi * 2)
            ...
            self._groundtruth_kitti_format.append(pd.DataFrame(predictions_kitti))
